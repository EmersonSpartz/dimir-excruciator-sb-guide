...
html=$(curl -sf "$URL")
for n in "Doomsday Excruciator" "Your grid" "pre-ban" "Kavaero" "Requiting Hex" "off by" "Cardsrealm" "postboard"; do
  grep -q "$n" <<< "$html" || { echo "FAIL: missing '$n'"; exit 1; }
done
mus=$(grep -o '<div class="mu\( open\)\?">' <<< "$html" | wc -l | tr -d ' ')
[ "$mus" -ge 19 ] || { echo "FAIL: expected >=17 sections, got $mus"; exit 1; }
